...
void SDFMap::initMap(const std::shared_ptr<rclcpp::Node>& nh) {
  node_ = nh;
   ...
   node_->declare_parameter<int>("sdf_map/depth_filter_margin", -1);
   node_->declare_parameter<double>("sdf_map/k_depth_scaling_factor", -1.0);
   node_->declare_parameter<int>("sdf_map/skip_pixel", -1);
   ...
   node_->declare_parameter<std::string>("sdf_map/frame_id", "world");
  ...
  depth_sub_.reset(new message_filters::Subscriber<sensor_msgs::msg::Image>(node_, "/sdf_map/depth", rclcpp::QoS(50).get_rmw_qos_profile()));

  if (mp_.pose_type_ == POSE_STAMPED) {
    pose_sub_.reset(new message_filters::Subscriber<geometry_msgs::msg::PoseStamped>(node_, "/sdf_map/pose", rclcpp::QoS(25).get_rmw_qos_profile()));
    ...
    odom_sub_.reset(new message_filters::Subscriber<nav_msgs::msg::Odometry>(node_, "/sdf_map/odom", rclcpp::QoS(100).get_rmw_qos_profile()));
    ...
    for (int y = min_esdf(1); y <= max_esdf(1); ++y)
      for (int z = min_esdf(2); z <= max_esdf(2); ++z) {
        ...
        if (md_.occupancy_buffer_inflate_[idx] == 0) {
          ...
          RCLCPP_ERROR(node_->get_logger(), "unexpected occupancy value");
        }
      }
...
void SDFMap::depthCallback(const sensor_msgs::msg::Image::ConstSharedPtr& img) {
  RCLCPP_INFO(node_->get_logger(), "depth ts: %u.%u", img->header.stamp.sec, img->header.stamp.nanosec);
}
 
void SDFMap::poseCallback(const geometry_msgs::msg::PoseStamped::ConstSharedPtr& pose) {
  RCLCPP_INFO(node_->get_logger(), "pose ts: %u.%u", pose->header.stamp.sec, pose->header.stamp.nanosec);
